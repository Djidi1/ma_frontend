...
new Vue({
  ...
  mounted: function() {
    ...
    this.auth_info = this.$ls.get("auth_info", {
      auth: false,
      name: "",
      email: ""
    });
  ...
  methods: {
    go_back() {
      ...
      if (element.find(".panel-left").hasClass("active")) {
        this.$f7.getCurrentView().activePage.name === "settings"
          ? this.$f7.views[0].back()
          : this.$f7.closePanel();
        return false;
      } else {
        if (
          this.$f7.getCurrentView().activePage.name === "audits_main" ||
          this.$f7.getCurrentView().activePage.name === "objects_main"
        ) {
      ...
      if (this.online) {
        ...
        this.synch_data(completed_audits).then(ready => {
          this.$root.getData_from_server().then(result => {
            self
              .return_uncompleted(in_progress_audits, result.obj)
              .then(complete => {
        ...
        this.$f7.alert(
          this.localization.no_connection,
          this.localization.pop_up.warning,
          function() {
            done();
          }
        );
    ...
    return_uncompleted(arr, result_arr) {
      ...
      return new Promise(function(resolve) {
        arr.forEach(function(itm) {
          let obj = self.$_.findWhere(result_arr, {
            id: Number(itm.object_id)
          });
          if (obj != undefined) {
      ...
      return new Promise(function(resolve) {
        ...
        result_sort.obj.sort(function(a, b) {
          return b.audits.hasOwnProperty(0) && a.audits.hasOwnProperty(0)
            ? new Date(b.audits[0].created_at) -
                new Date(a.audits[0].created_at)
            : false;
        });
        resolve(result_sort);
      });
    ...
    get_check_list() {
      ...
      return new Promise(function(resolve) {
        self.$http
          .post(
            self.be_server + "/api/get-checklists",
            {},
    ...
    get_objects() {
      ...
      return new Promise(function(resolve) {
        self.$http
          .post(
            self.be_server + "/api/get-objects",
            {},
            { headers: { Authorization: "Bearer " + self.auth_info.token } }
          )
          .then(response => {
    ...
    get_results() {
      ...
      return new Promise(function(resolve) {
        self.$http
          .post(
            self.be_server + "/api/get-results",
            {},
            { headers: { Authorization: "Bearer " + self.auth_info.token } }
          )
          .then(response => {
      ...
      return new Promise(function(resolve) {
        objects.forEach(function(object) {
          ...
          result = {
            ...
            audits: [],
            address: !object.audit_object_group
              ? ""
              : object.audit_object_group.address,
            group_id: !object.audit_object_group
              ? ""
              : object.audit_object_group.id,
            group_title: !object.audit_object_group
              ? "Нет группы"
              : object.audit_object_group.title
          };
          self.$_.filter(audits, { object_id: Number(object.id) }).forEach(
            function(audit) {
              let audit_result = {
                id: audit.id,
                title: audit.title,
    ...
    create_att_from_res(cl, audits, results) {
      ...
      let result = [];
      return new Promise(function(resolve) {
        cl.requirements.forEach(function(req) {
          let new_req = {
            id: req.id,
            title: req.title,
    ...
    get_status(audit, result, id) {
      ...
      result.forEach(function(itm) {
        res =
          itm.audit_id === audit.id
            ? itm.requirement_id === id
              ? itm.result
              : res
            : res;
      });
    ...
    ready_to_send(audit) {
      let self = this;
      this.$f7.confirm(
        this.$root.localization.modal.modalConfirmSend,
    ...
    send_data_to_sev(data, audit) {
      ...
      return new Promise(function(resolve, error) {
        self.create_promises(data).then(promises => {
          Promise.all(promises).then(values => {
            self.$http
              .post(self.be_server + "/api/put-audits", data, {
                headers: { Authorization: "Bearer " + self.auth_info.token }
              })
              .then(
                response => {
    ...
    get_Responsible() {
      ...
      this.$http
        .post(
          this.be_server + "/api/get-users",
          {},
          { headers: { Authorization: "Bearer " + this.auth_info.token } }
        )
        .then(response => {
