  ...
  <f7-page class="edit_audit_page">
    <f7-navbar back-link="Back" sliding @back-click.stop="close_edit()">
      <f7-nav-center sliding>
        <div v-if="!this.$root.online" style="display:inline-block; margin-right:2px">
          <network></network>
        </div>
        {{title}}
      </f7-nav-center>
          ...
          <div style="text-align: center">
            <div v-if="(this.audit_current.check_list.length>0)">
              <div v-if="!this.audit_current.error">
                <div v-if="(check_audit_status(this.audit_current))" class="new_audit_icon">
                  <new_audit_icon></new_audit_icon>
                </div>
                <div v-else>
                  <i v-if="allCheck(this.audit_current)" class="icon cloud_no_sink cloud"></i>
                  <div v-else class="new_audit_icon">
                    <new_audit_icon></new_audit_icon>
                  </div>
                </div>
              </div>
            ...
            <div v-else>
              <div class="no_check_list">
                <no_check_list></no_check_list>
              </div>
            </div>
          </div>
        </div>
    ...
    <f7-list form>
      <f7-list-item
        v-if="this.$root.check_list.length===0"
      ...
      >
        <div
          class="item-title no-padding no-white_spacing"
        >{{this.$root.localization.pop_up.check_list_group}}</div>
        <select :name="this.$root.localization.pop_up.add_check" @change="check_group_select_done">
          ...
          <option
            v-for="(check_gruop_item,check_ind) in check_list_groups"
            :key="check_ind"
            :value="check_gruop_item[0].cl_category.id"
          >{{check_gruop_item[0].cl_category.title}}</option>
          <div slot="after"></div>
        </select>
      </f7-list-item>
    </f7-list>
    ...
    <transition appear mode="out-in" name="slide-app">
      <f7-list form v-if="check_list_arr.length>0">
        ...
        >
          ...
          >{{this.$root.localization.pop_up.no_check_list}}</div>
        </f7-list-item>
        <f7-list-item
          v-else
          smart-select
        ...
        >
          <div
            class="item-title no-padding no-white_spacing"
          >{{this.$root.localization.pop_up.add_check}}</div>
          <select
          ...
          >
            <option
              v-for="(check_item,check_ind) in check_list_arr"
              :key="check_ind"
              :value="check_item.id"
              :selected="current_selected(check_item)"
            >{{check_item.title}}</option>
          </select>
          <div slot="after"></div>
        </f7-list-item>
      </f7-list>
...
export default {
  ...
  created() {
    let self = this;
    let obj = this.$_.findWhere(this.$root.objects, {
      id: Number(this.object_index)
    });
    this.audit_current = this.$_.findWhere(obj.audits, {
      id: Number(this.audit_id)
    });
    this.check_list_groups = this.$_.groupBy(
      this.$root.check_list,
      "cl_category_id"
    );
  ...
  methods: {
    ...
    check_group_select_done(item) {
      this.check_list_arr =
        item.target.value != null
          ? this.$_.filter(this.$root.check_list, function(c) {
    ...
    check_select_done(item) {
      ...
      arr.forEach(function(opt) {
        let check = self.$_.findWhere(self.$root.check_list, {
          id: Number($$(opt).val())
        });
        if (
          self.$_.findWhere(self.audit_current.check_list, {
            id: Number(check.id)
          }) === undefined
        ) {
          self.audit_current.check_list.push(
            self.add_check_list_to($$(opt).val())
          );
        }
      });
      let deletedCheckLists = self.$_.difference(
        Array.from(item.target.options),
        Array.from(item.target.selectedOptions)
      );
      this.audit_current.check_list.forEach(function(check, i) {
        if (
          self.$_.findWhere(deletedCheckLists, { _value: Number(check.id) }) !==
          undefined
        ) {
    ...
    add_check_list_to(key) {
      ...
      check.requirements.forEach(function(req) {
        let new_req = {
          id: req.id,
          title: req.title,
          status: 0,
    ...
    custome_smart_page() {
      let self = this;
      $$(document).on("pageBeforeInit", '[data-page*="smart-select"]', function(
        e
      ) {
        ...
        $$(page.container)
          .find(".navbar-inner")
          .append(self.get_html_button_done());
        $$(page.container)
          .find("a.done")
          .on("click", function(e) {
