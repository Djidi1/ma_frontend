  ...
  <f7-page>
    ...
    <div class="blck_info popup_card">
      <f7-list form>
        <f7-list-item
          smart-select
          :title="this.$root.localization.pop_up.select_ex"
        ...
        >
          <select :name="this.$root.localization.pop_up.select_ex" @change="selected_group_done">
            ...
            <option
              v-for="group in sorted_array"
              :key="group[0].group_id"
              :value="group[0].group_id"
            >{{group[0].group_title}}</option>
          </select>
      ...
      <transition appear mode="out-in" name="slide-app">
        <f7-list form v-if="this.sorted_array_objects.length>0">
          <f7-list-item
            smart-select
            :title="this.$root.localization.pop_up.select_pop_over"
            :smart-select-back-on-select="true"
            v-if="this.sorted_array_objects.length>0"
            :smart-select-searchbar="true"
            :smart-select-searchbar-placeholder="this.$root.localization.SearchBar.title"
            class="no-padding"
            id="sm-obj"
          >
            ...
            >
              ...
              <option
                v-for="(obj) in this.sorted_array_objects"
                :key="obj.id"
                :value="obj.id"
              >{{obj.title}}</option>
            </select>
      ...
      <transition appear mode="out-in" name="slide-app">
        <f7-list form v-if="Object.keys(selected_object).length > 0">
          ...
          <f7-list-item
            smart-select
            :title="this.$root.localization.pop_up.check_list_group"
            :smart-select-back-on-select="true"
            v-else
            :smart-select-searchbar="true"
            :smart-select-searchbar-placeholder="this.$root.localization.SearchBar.title"
          ...
          >
            ...
            >
              <option
                :value="null"
                selected
              >{{this.$root.localization.pop_up.no_check_cat_selected}}</option>
              <option
      ...
      <transition appear mode="out-in" name="slide-app">
        <f7-list
          form
          v-if="(check_list_arr.length > 0) && (Object.keys(selected_object).length > 0)"
        >
          <f7-list-item
            v-if="this.$root.check_list.length===0"
            :title="this.$root.localization.pop_up.no_check_list"
          ...
          <f7-list-item
            smart-select
            :title="this.$root.localization.pop_up.add_check"
            :smart-select-back-on-select="true"
            v-else
            :smart-select-searchbar="true"
            :smart-select-searchbar-placeholder="this.$root.localization.SearchBar.title"
...
export default {
  ...
  methods: {
    ...
    selected_object_done(item) {
      let self = this;
      let result_search = {};
      result_search = self.$_.findWhere(self.$root.objects, {
    ...
    add_audit() {
      let new_audit = {
        ...
        check_list: [],
        object_id:
          Object.keys(this.selected_object).length != 0
            ? this.selected_object.id
            : "Offline_" + this.getlastid(),
        upload: false,
        downloaded: false,
        completed: false,
        error: false
    ...
    custome_smart_page() {
      ...
      let curr_check_state = self.audits[0].check_list.map(val =>
        val.id.toString()
      );
      if (
        JSON.stringify(curr_check_state) !=
        JSON.stringify($$("#ch-lists").val())
      )
